/*
  ...
  Windows-friendly version (replaced BSD-specific calls, guarded strtok_r).
*/
		    ...
		    int buflen)
{
     ...
     if (parse->path == NULL) {          // replace empty abs_path with "/"
	  int rlen = (int)strlen(root_abs_path);
	  ...
	  int rlen = (int)strlen(root_abs_path);
	  int plen = (int)strlen(parse->path);
     ...
     if (parse->port != NULL) {
	  int port = (int)strtol (parse->port, (char **)NULL, 10);
